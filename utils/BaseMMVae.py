...
class BaseMMVae(ABC, nn.Module):
    ...
    def reparameter_by_weight(self, weight, mus, logvar):
        """

        :param weight: batch_size * num_of_subset
        :param mus: num_of_subset * batch_size * latent_dim
        :param logvar: num_of_subset * batch_size * latent_dim
        :return:
        """
        z_compelete = self.reparameterize(mus[-1], logvar[-1])  # batch_size * latent_dim
        z_target = torch.zeros((mus.shape[1], mus.shape[2])).to(self.flags.device)  # batch_size * latent_dim

        # print(weight.shape, mus.shape, logvar.shape)
        for sample_index in range(weight.shape[0]):
            target_mu, target_logvar = self.moe_fusion_sample(mus[:, sample_index, :], logvar[:, sample_index, :], weight[sample_index, :])
            # print(target_mu.shape, target_logvar.shape)
            z_target[sample_index, :] = self.reparameterize(target_mu, target_logvar)
            # print(target_mu.shape, target_logvar.shape)

        return z_compelete, z_target

    def moe_fusion_sample(self, mus, logvars, weights=None):
        mu_moe = torch.zeros_like(mus[0])
        logvar_moe = torch.zeros_like(logvars[0])
        for index in range(mus.shape[0]):
            mu_moe = mu_moe + weights[index] * mus[index, :]
            logvar_moe = logvar_moe + weights[index] * logvars[index, :]
    ...
    def label2weights(self, label):
        """
        :param label:
        :return:
        """
        weights = torch.zeros((len(label), 7)).to(self.flags.device)
        for sample in range(len(label)):
            if label[sample]:
                weights[sample, :] = torch.tensor([1, 0, 1, 0, 1, 0, 0]).to(self.flags.device)
            else:
                weights[sample, :] = torch.tensor([1, 1, 0, 1, 0, 0, 0]).to(self.flags.device)
    ...
    def forward_mod_miss(self, input_batch, label=None):
        latents = self.inference(input_batch, None, label);
        results = dict();
        class_embeddings = self.reparameterize(latents['joint'][0],
                                               latents['joint'][1]);
        results['class_embeddings'] = class_embeddings  # z samples from multi-modal posterior distribution
        return results

    def forward(self, input_batch, label=None):
        latents = self.inference(input_batch, None, label);
        results = dict();
        results['latents'] = latents;
        results['group_distr'] = latents['joint'];  # joint_mu, joint_logvar
        class_embeddings = self.reparameterize(latents['joint'][0],
                                                latents['joint'][1]);
        results['class_embeddings'] = class_embeddings  # z samples from multi-modal posterior distribution

        # z sample from target subset distribution
        temp_mus = torch.Tensor().to(self.flags.device);
        temp_logvars = torch.Tensor().to(self.flags.device);
        for key, value in latents['subsets'].items():
            s_mu = value[0]
            s_logvar = value[1]
            temp_mus = torch.cat((temp_mus, s_mu.unsqueeze(0)), dim=0);
            temp_logvars = torch.cat((temp_logvars, s_logvar.unsqueeze(0)),
                                dim=0);
        _, z_target = self.reparameter_by_weight(latents['weights4target'], temp_mus, temp_logvars)
        # _, z_target = self.reparameter_by_weight(latents['weights4target'], latents['mus'], latents['logvars'])
        results['latent_complete'] = class_embeddings
        results['latent_target'] = z_target

        weights = torch.tensor([1, 1, 1, 1, 1, 1, 1]).to(self.flags.device)
        ...
        for m, m_key in enumerate(self.modalities.keys()):
            if m_key in input_batch.keys():
                ...
                # mean = x_hat , std = 1
                # m_rec = self.lhoods[m_key](self.decoders[m_key](m_s_embeddings, class_embeddings), 1);
                # results_rec[m_key] = m_rec;
                rec_data[m_key] = self.decoders[m_key](m_s_embeddings, class_embeddings)  # \hat_{x}
        # results['rec_distribution'] = results_rec;
        results['rec_data'] = rec_data;
        return results;
    ...
    def encode(self, input_batch):
        ...
        for m, m_key in enumerate(self.modalities.keys()):
            if m_key in input_batch.keys():
                ...
                latents[m_key + '_style'] = [None, None]  # style_mu, style_logvar
                latents[m_key] = l  # class_mu, class_logvar
            else:
    ...
    def inference(self, input_batch, num_samples=None, label=None):
        """
        ...
        :return:
            latents['modalities']: {
                "m0": [class_mu, class_logvar]
                "m1": [class_mu, class_logvar]
                "m2": [class_mu, class_logvar]
                "m0_style": [style_mu, style_logvar]
            }
            latents['mus']:
            latents['logvars']:
            latents['weights']:
            latents['joint']: =joint_mu, joint_logvar = self.moe_fusion(mus, logvars, weights)
            latents['subsets']:
        """
            ...
            if s_key != '':
                ...
                for m, mod in enumerate(mods):
                    if mod.name in input_batch.keys():
                        # mus_subset: [subset_num , batch, data]
                        mus_subset = torch.cat((mus_subset,
                                                enc_mods[mod.name][0].unsqueeze(0)),
                                               dim=0);
                        # logvars_subset: [subset_num , batch, data]
        ...
        weights = torch.tensor([1, 0, 0, 1, 1, 0, 1]).to(self.flags.device)
        if self.flags.modality_moe:
            weights = torch.tensor([1.0, 1.0, 1.0]).to(self.flags.device)
        elif self.flags.modality_poe:
            weights = torch.tensor([1.0]).to(self.flags.device)
        elif self.flags.joint_elbo:
            weights = torch.tensor([1.0, 0, 0, 1.0, 1.0, 0, 1.0]).to(self.flags.device)


        # weights = torch.tensor([0, 1, 1, 0, 0, 1, 0]).to(self.flags.device)
        weights4target = self.label2weights(label)
        # weights = torch.tensor([1, 1, 1, 1, 1, 1, 1]).to(self.flags.device)
        ...
        latents['subsets'] = distr_subsets;  # {"m0": [mu, logvar], ..., "m0_m1_m2": [mu, logvar]}
        latents['weights4target'] = weights4target
        return latents;
